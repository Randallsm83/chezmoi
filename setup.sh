...
install_base_packages() {
    ...
    if command_exists pacman; then
        ...
        # Pi/aarch64-Debian homelab bootstrap: zsh-only. Do not pull build deps
        # unless the user opts into managed package installs later.
        if [ "${RASPI:-0}" = "1" ]; then
            log_info "Raspberry Pi detected - installing only zsh essentials"
            packages="git curl wget unzip zip zsh zsh-autosuggestions zsh-syntax-highlighting"
        fi
...
# ============================================================================
# Raspberry Pi: seed .chezmoi.local.toml with homelab zsh-only overrides
# ============================================================================
...
seed_raspi_local_toml() {
    ...
    log_info "Seeding $local_toml with Raspberry Pi homelab zsh-only overrides"
    cat > "$local_toml" <<'EOF'
# Auto-generated by setup.sh on a Raspberry Pi (RASPI=1).
# Edit freely - this file is git-ignored and survives chezmoi apply.
[data]
    is_raspi = true
    # Keep the medium remote tier for the lightweight zsh loader, but do not
    # install the medium package/tool set unless explicitly opted in below.
    remote_tier = "medium"
    ...
    [data.package_features]
        ...
        starship = false
        fzf = false
        rust_alternatives = false
        thefuck = false
        fastfetch = false
        # languages off by default; enable only what this host needs
        node = false
        python = false
        rust = false
        golang = false
...
install_and_apply_dotfiles() {
    ...
    log_info "This will configure your environment; managed tool installs depend on install_packages"
