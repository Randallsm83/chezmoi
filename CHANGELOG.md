...
- **OMP homelab auth-broker/gateway helpers + shared agent config**: new `dot_omp/agent/config.yml.tmpl` makes `~/.omp/agent/config.yml` chezmoi-managed across Windows and WSL with a single shared baseline (provider/web-search/image choices, status-line + display styling, model roles, thinking/steering/follow-up behavior, STT, compaction handoff, mnemopi scoping), keeping only genuinely platform-specific entries (e.g. WSL `disabledExtensions: [mcp:pam]`) behind an `isWSL` guard. A companion `dot_omp/agent/dot_env.tmpl` now renders runtime env cross-platform: Windows/Git Bash uses `BASH_ENV=~/.config/omp/agent-bash-env` and zsh hosts use `OMP_AGENT_SHELL=1` so non-interactive OMP commands inherit the managed PATH, aliases, functions, and MCP timeout without loading interactive shell startup. Shell wrappers gain homelab auth helpers: zsh `dot_config/zsh/dot_zshrc.d/25-functions.zsh` and pwsh `Documents/PowerShell/Scripts/lib/99-functions-body.ps1` define `ompb`/`ompg` (run `omp auth-broker`/`auth-gateway` inside the Pi containers over SSH), `ompb-login`, `ompg-url`, and `ompg-models`, all resolving the auth host (`raspi`) and public gateway base URL via overridable `OMP_AUTH_HOST` / `OMP_GATEWAY_PUBLIC_BASE_URL`. The `omp` wrapper now prefers a synchronized local `~/.omp/auth-broker.token` (setting `OMP_AUTH_BROKER_URL` + `OMP_AUTH_BROKER_TOKEN` for that process only) and falls back to the `op run` env-file path when the token file is absent, so Windows and WSL hit the same broker.
...
- **Standalone Qdrant/Tavily MCP config**: added `dot_mcp.json.tmpl` so `~/.mcp.json` exposes `qdrant` and `tavily` directly across Windows, Linux, and macOS without enabling pam. Claude Code gets the same standalone entries via `run_after_71_claude-code_mcp_sync_{windows,unix}` because `~/.claude.json` is runtime-managed and intentionally ignored by chezmoi.
