...
### Changed
- **`Documents/PowerShell/Scripts/99-aliases.ps1`** — workspace env vars rewritten from hardcoded `D:\DH Dev` / `D:\DH Dev\projects` to cross-platform `Join-Path $HOME 'projects' …` form. The phantom `D:\DH Dev` directory is no longer referenced.
