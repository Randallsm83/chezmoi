...
## [Unreleased]
### Security
- **Leaked credentials and infrastructure identifiers scrubbed from the public repo.** Removed real DreamHost work API keys (14× Locize, 1× Firebase + project), the OMP auth-broker URL, internal/tailnet hostnames (DreamHost servers, DreamHost shell VPS, DreamPress host, Raspberry Pi tailnet FQDN, LM Studio Mac host), private IPs (Pritunl VPN resolver, LAN, tailnet), remote usernames, and personal full-name PII from tracked files. Genuine secrets resolve via 1Password (`op://` refs / `op run`); infrastructure identifiers are parameterized through a new `[data.infra]` table sourced from gitignored `.chezmoi.local.toml` (empty defaults in public source), consumed via `{{ with .infra.<key> }}` guards so a missing value degrades gracefully. Plain shell files fall back to existing env vars (`OMP_AUTH_HOST`, etc.). Docs genericized to placeholders. See `SECRETS.md` → "Infrastructure identifiers". **The exposed API keys were public in git history and MUST be rotated at their providers (Locize, Firebase) — scrubbing/​history-rewrite does not undo prior exposure.**
- **Secret-scanning defense-in-depth added.** A `gitleaks` pre-commit hook (`.githooks/pre-commit`, wired via `core.hooksPath` by `run_onchange_after_76_install-git-hooks`) blocks commits containing secrets before they leave the machine, using a repo `.gitleaks.toml` (built-in ruleset plus explicit Locize-UUID and credential-literal rules, with `op://`/`{{ ... }}`/`<placeholder>` allowlists). The hook is fail-closed but only activates where `gitleaks` is installed (added to the `git` package feature: scoop on Windows, mise on Linux, brew on macOS), so scanner-less hosts are never locked out. GitHub-native secret scanning + push protection are enabled on the repo, with CI backstops on both mirrors (`.github/workflows/secret-scan.yml` gitleaks Action; `.gitlab-ci.yml` GitLab Secret Detection).

### Fixed
- **Standalone Qdrant MCP durability**: removed the stale qdrant server from the unmanaged PAM manifest, kept qdrant in the standalone `~/.mcp.json` flow, and rendered `HF_HUB_OFFLINE=0` plus a persistent FastEmbed cache path so `mcp-server-qdrant` does not fail after `%TEMP%` cleanup.

- **OMP local broker-token wrapper**: zsh and PowerShell `omp` wrappers now prefer `~/.omp/auth-broker.token`, set `OMP_AUTH_BROKER_URL` to `<broker-url>` when unset, and only fall back to `op run ~/.config/op/omp.env` when the local token is unavailable. This keeps `omp` working without a 1Password unlock/prompt loop.
...
- **Encrypted DNS profile (macOS)**: New `encrypted_dns` block in `.chezmoidata.yaml` plus `dot_config/dns/private_pihole-dot.mobileconfig.tmpl` and `.chezmoiscripts/run_onchange_after_56_encrypted-dns.sh.tmpl` install a `com.apple.dnsSettings.managed` profile pinning the system resolver at `raspi.<your-tailnet>.ts.net:853` over DoT. TCP-probes the endpoint before installing; skips with a warning if the Pi-side terminator isn't up yet. Encrypts the LAN leg of DNS that was previously plaintext UDP/53.
...
- **`dot_config/zsh/dot_zshrc.d/{10-dirs,25-aliases}.zsh`** — removed hardcoded `/home/<user>/projects` from `cdp` and `dhgitall`; repo shortcuts now derive from `$DHSPACE` / `$BACKEND`.
